...
function sanitize(cursor: Cursor, chapters: ChapterDef[]): Cursor {
  ...
  const stepCount = chapters[chapter]!.steps.length;
...
export function useStepper(chapters: ChapterDef[]): StepperState {
  ...
  const totalGlobal = useMemo(
    () => chapters.reduce((s, c) => s + c.steps.length, 0),
    [chapters],
  );
  ...
  const prev = useCallback(() => {
    setCursor((cur) => {
      ...
      if (cur.chapter > 0) {
        ...
        return { chapter: cur.chapter - 1, step: p.steps.length - 1 };
      }
      return cur;
    });
  }, [chapters]);
  ...
  const jumpToChapter = useCallback(
    (idx: number, step = 0) => {
      ...
      setCursor({
        chapter: ch,
        step: clamp(step, 0, c.steps.length - 1),
      });
    },
    [chapters],
  );
  ...
  const jumpToGlobal = useCallback(
    (g: number) => {
      ...
      for (let i = 0; i < chapters.length; i++) {
        const t = chapters[i]!.steps.length;
        if (target < acc + t) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight" || e.key === " ") {
        ...
        jumpToChapter(last, chapters[last]!.steps.length - 1);
  ...
  return {
    ...
    chapterTotalSteps: ch.steps.length,
    globalIndex,
    totalGlobal,
    next,
    prev,
